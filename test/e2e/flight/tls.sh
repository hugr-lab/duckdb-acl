...
# one-shot: the serve errors and the CLI exits; we assert the reason
clear_out="$( { setup; echo "SELECT acl_flight_serve('grpc://0.0.0.0:$PORT');"; } | "$DUCKDB" -unsigned 2>&1 || true )"
case "$clear_out" in
...
# a client that does NOT trust the cert cannot connect (TLS verifies the server)
got="$(python3 "$HERE/client.py" "grpc+tls://localhost:$PORT" "SELECT 1" "$TOKEN" 2>&1 || true)"
case "$got" in *[Cc]ertificate*|*andshake*|*SSL*|*TLS*|*[Uu]navailable*|*UNAVAILABLE*|*[Ff]ailed*) ;; *) fail "an untrusting client was not rejected by TLS: $got";; esac
