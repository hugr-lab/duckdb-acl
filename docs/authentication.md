...
token is rejected, the cached document is not used either, and a `keys` event `refresh_failed`
carries the reason - a policy catalog shared by a fleet cannot make one node read what it was not
told to. The prefixes are compared as written: lower-case scheme, a directory prefix ending in `/`.
...
keys, never the keys). `acl_jwks_refresh([issuer])` drops the cached document(s) and answers how many,
so the next token re-reads whatever the refresh interval says - the rotation-incident verb. Both are
the operator's: denied to a principal, like every `acl_*` function.
...
| `token rejected: the keys of issuer "X" are at "uri" is outside acl_jwks_locations (…)` | the location is not on this node's list (spec 071) | list its prefix in `acl_jwks_locations`, or paste the keys |
