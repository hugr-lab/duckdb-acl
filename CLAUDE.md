...
**Spec 071**: `acl_jwks_locations` (GLOBAL, default `https://`) lists the prefixes a `KEYS FROM`
location may start with; a location outside it is refused where written and again where read (the
node's setting binds, whatever a shared catalog says; a `keys` event carries the reason);
`acl_jwks_cache()` lists what the node trusts (location, allowed, fetched/tried, error, key count,
kids - never keys), `acl_jwks_refresh([issuer])` drops the cache so the next token re-reads.
