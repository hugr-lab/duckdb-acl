...
idx_t QuackFetcher::GetReadAheadDepth(ClientContext &context) {
	Value val;
	idx_t depth = 0;
	if (context.TryGetCurrentSetting("acl_quack_fetch_read_ahead", val) && !val.IsNull()) {
...
QuackFetcher::QuackFetcher(ClientContext &context, QuackClientConnection &connection, hugeint_t query_uuid,
	...
	Value delay_val;
	if (context.TryGetCurrentSetting("acl_quack_debug_fetch_delay_ms", delay_val) && !delay_val.IsNull()) {
