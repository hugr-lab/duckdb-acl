...
static unique_ptr<FunctionData> QuackScanFromClientBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (!session_state) {
		throw InvalidInputException("acl_quack_scan_data is an internal function driven by the quack server");
	}
	...
	if (prototype.id() != LogicalTypeId::STRUCT) {
		throw InvalidInputException("acl_quack_scan_data: the second argument must be a STRUCT prototype, for example "
	...
	if (types.empty()) {
		throw InvalidInputException("acl_quack_scan_data: the STRUCT prototype has no columns");
	}
